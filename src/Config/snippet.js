{
	...
	"SClassComponentRedux": {
		...
		"body": [
			...
			"\t\t\t\t\t{props.${2:name}.list.length > 0 ? props.${2:name}.list.map((item, index) => { return <React.components. key={index} index={index} object={item} /> }) : <React.components.Spinner />}",
			"\t\t\t\t</div>\n",
	...
	"SFunctionComponentForm": {
		...
		"body": [
			...
			"\tconst submit = values => isNewUpadte${1:functionName}() ? React.actions.actions${1:functionName}.create${1:functionName}(values) : React.actions.actions${1:functionName}.update${1:functionName}(values, Object.keys(current${1:functionName})[0]);\n",
			"\tconst remove = () =>{",
			"\t\tprops.clear();",
			"\t\tReact.config.storeHistory.history.goBack();",
			"\t\tReact.actions.actions${1:functionName}.remove(Object.keys(selected)[0]);",
			"\t}\n",
			"\treturn (",
