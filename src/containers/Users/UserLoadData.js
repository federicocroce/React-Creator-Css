...
const UserLoadDataReduxForm = props => {
    ...
    return (
        <form onSubmit={props.handleSubmit(submit.bind(this))}>
            ...
            <InputText name='name' style='inline' placeholderFloating='Nombre' customPlaceholder='Feerico Croce' type='text' validate={React.config.fieldValidations.validations.name} />

            <InputText name='age' style='inline' placeholderFloating='Edad' customPlaceholder='28' type='text' validate={React.config.fieldValidations.validations.age} />
            ...
            <Button className='primary-button' label='Eliminar' onClick={() => remove(selected, props)} />
            <Button className='primary-button' label='Reset' onClick={() => { props.setSelected({}); reset() }} />
            <Button className='primary-button' label='VOLVER' onClick={() => props.setSelected({})} back />
        </form>
    );
}
